...
class FoldingNetDecoder(nn.Module):
    def __init__(self, feat_dims: int, shape="sphere") -> None:
        super().__init__()
        self.m = 2048  # 45 * 45.
        self.shape = shape
...
class SkipValiationalFoldingNet(nn.Module):
    ...
    def get_loss(self, point, recon, recon2, mu, sigma):
        loss_emd = torch.mean(self.emd(recon, point))
        loss_emd_2 = torch.mean(self.emd(recon2, point))
        dist1, dist2, idx1, idx2 = self.chd(point, recon)
        loss_cd = (torch.mean(dist1)) + (torch.mean(dist2))
        loss_kl = -0.5 * torch.sum(1 + sigma - mu.pow(2) - sigma.exp())
        return loss_emd + loss_emd_2, loss_cd, loss_kl


class Projection(nn.Module):
    def __init__(self):
        super(Projection, self).__init__()
        self.fc = nn.Linear(in_features=512, out_features=512, bias=False)
...
class Decomposer(nn.Module):
    def __init__(self, num_parts):
        ...
        self.projection_layer_list = nn.ModuleList([Projection() for _ in range(num_parts)])

    def forward(self, inputs):
        projection_layer_outputs = [layer(inputs) for layer in self.projection_layer_list]
        outputs = torch.stack(projection_layer_outputs, dim=1)
        return outputs


class PartDecoder(nn.Module):
    def __init__(self, feat_dims: int, num_point: int, shape="sphere") -> None:
        super().__init__()
        self.m = num_point
        self.shape = shape
        # self.sphere = np.load(f"./grids/sphere_2048.npy")
        # self.gaussian = np.load("./grids/gaussian.npy")
    ...
    def build_grid(self, num_point, batch_size: int) -> torch.tensor:
        if self.shape == "sphere":
            points = self.sample_sphere(num_point)
        elif self.shape == "gaussian":
            points = self.gaussian
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        x = x.transpose(1, 2).repeat(1, 1, self.m)
        points = self.build_grid(self.m, x.shape[0]).transpose(1, 2)
...
class PartVFoldingNet(nn.Module):
    def __init__(self, n_points=2048, feat_dims=512, num_parts=4):
        super().__init__()
        self.encoder = SkipVariationalEncoder(n_points, feat_dims)
        self.num_point = n_points // num_parts
        self.part_decoder = PartDecoder(feat_dims, num_point=self.num_point, shape='sphere')
        self.decomposer = Decomposer(num_parts=4)
    ...
    def forward(self, input: torch.tensor):
        ...
        sigma = sigma.transpose(2, 1)
        feature = self.sample_z(mu, sigma)
        decomposer_output = self.decomposer(feature)

        decoder_outputs = list()
        for each in decomposer_output:
            folding2, _ = self.part_decoder(each)
            decoder_outputs.append(folding2)
        return decoder_outputs, mu, sigma

    def get_loss(self, point, recon, mu, sigma):
        loss_emd = torch.mean(self.emd(recon, point))
        dist1, dist2, idx1, idx2 = self.chd(point, recon)
        loss_cd = (torch.mean(dist1)) + (torch.mean(dist2))
        loss_kl = -0.5 * torch.sum(1 + sigma - mu.pow(2) - sigma.exp())
        return loss_emd, loss_cd, loss_kl


if __name__ == '__main__':
    model = PartVFoldingNet(n_points=2048, feat_dims=512, num_parts=4).to('cuda')
    points = torch.randn(1, 3, 2048).to('cuda')
    decoder_outputs, mu, sigma = model(points)
    print(decoder_outputs.shape, mu.shape, sigma.shape)
