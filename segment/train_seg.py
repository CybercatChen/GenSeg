...
def train(args, config, summarywriter):
    ...
    for epoch in range(config.train.max_epoch):
        ...
        summarywriter.add_scalar('Loss/Epoch/0_loss_emd', losses.avg(0), epoch)
        summarywriter.add_scalar('Loss/Epoch/1_loss_ss', losses.avg(1), epoch)
        summarywriter.add_scalar('Loss/Epoch/2_oss_loc', losses.avg(2), epoch)
        summarywriter.add_scalar('Loss/Epoch/3_loss_sp_balance', losses.avg(3), epoch)
        summarywriter.add_scalar('Loss/Epoch/4_all_loss', losses.avg(4), epoch)
...
def train_one_epoch(args, config, model, train_loader, optimizer, criterion, epoch, summarywriter):
    losses = utils.AverageMeter(['loss_fit', 'loss_ss', 'loss_loc', 'loss_sp_balance', 'all_loss'])
    # losses = AverageMeter(['loss_ss', 'loss_loc', 'loss_sp_balance', 'all_loss'])
    ...
    for i, data in enumerate(train_loader):
        ...
        summarywriter.add_scalar('Loss/Batch/0_loss_emd', loss_emd.item(), n_itr)
        summarywriter.add_scalar('Loss/Batch/1_loss_ss', loss_ss.item(), n_itr)
        summarywriter.add_scalar('Loss/Batch/2_loss_loc', loss_loc.item(), n_itr)
        summarywriter.add_scalar('Loss/Batch/3_loss_sp_balance', loss_sp_balance.item(), n_itr)
        summarywriter.add_scalar('Loss/Batch/4_all_loss', loss.item(), n_itr)
